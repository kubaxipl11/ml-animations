        ...
        if (Math.abs(userAnswer - correctAnswer) < 0.01) {
            ...
            setTimeout(() => {
                if (currentStep < PRACTICE_STEPS.length - 1) {
                    ...
                    setIsComplete(true);
                    setFeedback('🎉 Perfect! You completed QR decomposition!');
                }
            }, 1000);
        } else {
        ...
        <div className="flex flex-col items-center p-3 h-full">
            ...
            {!isComplete ? (
                <div className="mt-3 w-full max-w-sm">
                    ...
                    {feedback && (
                        <div className={`mt-2 p-3 rounded-lg text-center font-bold ${feedback.includes('✓') ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>{feedback}</div>
                    )}
                </div>
            ) : (
